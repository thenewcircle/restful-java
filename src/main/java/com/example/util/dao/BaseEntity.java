...
public abstract class BaseEntity implements Entity, Serializable {
	...
	/**
	 * The cacheKey should be unchangeable once set. This ensures an object will
	 * always be stored in a consistent location in a HashMap. By default id is
	 * used, but if a more suitable immutable value exists, subclasses may use
	 * it.
	 * 
	...
	public boolean equals(Object o) {
		...
		Object thisId = this.getCacheKey();
		Object otherId = other.getCacheKey();
		if (thisId == null) {
			return false;
		}
		if (otherId == null) {
	...
	public String toString() {
		...
		try {
			...
			if (cacheKey != id) {
				String result = name + "[id=" + id + ", key=" + cacheKey + "]";
				return result;
			}
		} catch (Exception e) {
			e.printStackTrace();
