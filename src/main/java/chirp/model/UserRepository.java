...
/**
 * Data access object for users. Note that the user repository also manages bulk deletion of users,
 * which will be covered when modeling RESTful transactions. This class is thread-safe.
 */
public class UserRepository implements Serializable {
    ...
    /**
     * Use this method to create a new UserRespository suitable for unit testing if a repository
     * does not exist. Always return an existing repository.
     *
     ...
     *            Create a repository if one has not been create already according the value of the
     *            parameter. If a repository has not been created and favorPersistence equals false,
     *            create an empty repository, otherwise, if true, create a repository from the
     *            state.bin file if it exists. If the file does not exist, create an empty new
     *            repository.
     * @return a UserRepistory.
     */
    public static synchronized UserRepository getInstance(boolean favorPersistence) {
        if (instance == null) {
    ...
    /**
     * Call this method to create user repository from the file <code>state.bin</code> if it exists.
     *
     * @return
     */
    ...
    public User createUser(String username, String realname) {
        ...
        final User user = new User(username, realname);
    ...
    public boolean commitBulkDeletion(int id) {
        try {
            final Set<User> bulkDeletion = bulkDeletions.get(id);
            for (final User user : bulkDeletion) {
