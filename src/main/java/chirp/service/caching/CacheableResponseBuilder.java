...
public class CacheableResponseBuilder {
	...
	private ResponseBuilder evalutePreConditions() {
		final ResponseBuilder rb;
		if (cacheableResource != null) {
			if (cacheableResource.getEntityTag() != null
					&& cacheableResource.getLastModificationTime() != null) {
				rb = this.request.evaluatePreconditions(
						cacheableResource.getLastModificationTime(),
						cacheableResource.getEntityTag());
			} else if (cacheableResource.getEntityTag() != null) {
				rb = this.request.evaluatePreconditions(cacheableResource
						.getEntityTag());
	...
	public Response build() {
		...
		case HttpMethod.HEAD:
			ResponseBuilder rb = evalutePreConditions();
