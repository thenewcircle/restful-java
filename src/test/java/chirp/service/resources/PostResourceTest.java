...
public class PostResourceTest extends JerseyResourceTest {
	...
	public void createChirpSuccess() {
		...
		response = getEntity(response.getLocation()
				.getPath(), MediaType.APPLICATION_JSON_TYPE, Response.Status.OK);

		PostRepresentation postRead = readEntity(response, PostRepresentation.class);

		assertNotNull(postRead); // make sure the GET response contains an
									// entity.
		
		assertEquals("unit test success?", postRead.getContent()); // validate the
	...
	public void readPostCollectionSuccess() {
		...
		for (PostRepresentation post : readEntity("/posts/gordonff",
			...
			getHead(post.getSelf(), MediaType.APPLICATION_JSON_TYPE, Response.Status.OK);
		}
	...
	public void verifyPostCollectionLinkHeaders() {
		...
		Response response = getHead("/posts/gordonff", MediaType.APPLICATION_JSON_TYPE, Response.Status.OK);
		verifyLinkHeaderExists("self", MediaType.APPLICATION_JSON_TYPE, response);
